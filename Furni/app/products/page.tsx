  ...
  return (
    <div className="p-6 bg-[#f5f5dc]">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Filters Section */}
        <aside className="bg-[#8B4513] p-4 shadow rounded text-white">
          ...
          <button className="w-full bg-[#A0522D] text-white py-2 rounded mt-4 hover:bg-[#8B4513]">
            Apply Filters
          </button>
        </aside>
        ...
        <section className="lg:col-span-3">
          <h2 className="text-2xl font-semibold mb-6 text-[#8B4513]">Furnitures</h2>
            ...
            {filteredProducts.map((product) => (
              <div key={product._id} className="bg-white p-4 shadow rounded flex flex-col h-full">
                <Link href={`/product/${product.slug.current}`} className="flex flex-col flex-grow">
                  ...
                  <h3 className="text-lg font-medium mt-4 text-[#8B4513]">{product.name}</h3>
                  <p className="text-gray-600">Price ${product.price}</p>
                  {/* Add to Cart Button at Bottom */}
                  <div className="mt-auto">
                    <button
                      onClick={(e) => handleAddToCart(e, product)}
                      className="w-full bg-[#8B4513] text-white py-2 rounded h-10 hover:bg-[#A0522D]"
                    >
                      Buy Now
                    </button>
                  </div>
